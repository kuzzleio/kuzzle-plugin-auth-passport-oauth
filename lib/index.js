...
class PluginPassportOAuth {
  ...
  verify (request, accessToken, refreshToken, profile) {
    ...
    return this.getCredentialsFromKuid(profile._json[this.config.strategies[profile.provider].identifierAttribute], profile.provider)
      .then(userObject => {
        let req = null;
        ...
        if (this.config.strategies[profile.provider].persist.length !== 0) {
          ...
          req = {
            controller: 'security',
            action: 'createUser',
            _id: profile._json[this.config.strategies[profile.provider].identifierAttribute],
            body: {
              content: {
          ...
          if (this.config.strategies[profile.provider].kuzzleAttributesMapping) {
            Object.keys(this.config.strategies[profile.provider].kuzzleAttributesMapping)
              .forEach(attr => {
                req.body.content[attr] = profile._json[this.config.strategies[profile.provider].kuzzleAttributesMapping[attr]];
              });
          }

          user._id = profile._json[this.config.strategies[profile.provider].identifierAttribute];
          ...
          return this.context.accessors.execute(new this.context.constructors.Request(request.original, req))
            .then(() => {
              return Promise.resolve(profile._json[this.config.strategies[profile.provider].identifierAttribute]);
            });
        }
        return Promise.resolve(null);
      })
   ...
   * @returns {Promise|Promise.<TResult>}
   */
  validate (request, credentials, kuid, provider) {
